...
class Request {
	...
	request(options = {}) {
		// #ifdef APP-PLUS
		// 监听设备网络状态变化事件
		plus.globalEvent.addEventListener('netchange', (e)=>{console.log(e)});
		// #endif
		// 检查请求拦截
		...
		return new Promise((resolve, reject) => {
			options.complete = (response) => {
				...
				if(this.config.originalData) {
					...
					if (this.interceptor.response && typeof this.interceptor.response === 'function') {
						...
						// 如果拦截器不返回false，就将拦截器返回的内容给this.$w.post的then回调
						if (resInterceptors !== false) {
							resolve(resInterceptors);
						} else {
							// 如果拦截器返回false，意味着拦截器定义者认为返回有问题，直接接入catch回调
							// reject(response);//省市县接口返回被拦截
						}
					} else {
			...
			// 如果返回reject()，不让其进入this.$w.post().then().catch()后面的catct()
