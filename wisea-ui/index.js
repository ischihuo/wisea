...
// 尝试判断在根目录的/store中是否有$w.mixin.js，此文件uView默认为需要挂在到全局的vuex的state变量
// HX2.6.11版本,放到try中,控制台依然会警告,暂时不用此方式，
// let vuexStore = {};
// try {
// 	vuexStore = require("@/store/$w.mixin.js");
// } catch (e) {
...
// 新增图片转base64及base64转图片
import { pathToBase64, base64ToPath } from './libs/function/image-tools.js'

import { getDictLabel, getDictType } from './libs/function/dictFormat.js'
...
// 接口文件
import {getApi} from './libs/function/api.js'
// 上传
import {uploadFile} from './libs/function/uploadFile.js'
...
const install = Vue => {
	...
	// 获取设备信息，挂载到$w的sys(system的缩写)属性中，
	// 同时把安卓和ios平台的名称"ios"和"android"挂到$w.os中，方便取用
	$w.sys = uni.getSystemInfoSync();
	$w.os = $w.sys.platform;
	Vue.prototype.$w = $w
}
