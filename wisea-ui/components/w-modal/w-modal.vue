<template>
	<view>
		<w-popup :zoom="zoom" mode="center" :popup="false" :z-index="uZIndex" v-model="value" :length="width"
		 :mask-close-able="maskCloseAble" :border-radius="borderRadius" @close="popupClose" :negative-top="negativeTop">
			<view class="w-model">
				<view v-if="showTitle" class="w-model__title w-line-1" :style="[titleStyle]">{{ title }}</view>
				<view class="w-model__content">
					<view :style="[contentStyle]" v-if="$slots.default">
						<slot />
					</view>
					<view v-else class="w-model__content__message" :style="[contentStyle]">{{ content }}</view>
				</view>
				<view class="w-model__footer w-border-top" v-if="showCancelButton || showConfirmButton">
					<view v-if="showCancelButton" :hover-stay-time="100" hover-class="w-model__btn--hover" class="w-model__footer__button"
					 :style="[cancelBtnStyle]" @tap="cancel">
						{{cancelText}}
					</view>
					<view v-if="showConfirmButton" :hover-stay-time="100" :hover-class="asyncClose ? 'none' : 'w-model__btn--hover'"
					 class="w-model__footer__button hairline-left" :style="[confirmBtnStyle]" @tap="confirm">
						<w-loading mode="circle" :color="confirmColor" v-if="loading"></w-loading>
						<block v-else>
							{{confirmText}}
						</block>
					</view>
...
<script>
	 ...
	 * @example <w-modal :src="title" :content="content"></w-modal>
	 */
	export default {
		name: 'w-modal',
		props: {
