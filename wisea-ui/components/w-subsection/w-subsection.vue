<template>
	<view class="w-subsection" :style="[subsectionStyle]">
		<view class="w-item w-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'w-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="w-item-text w-line-1">{{ item.name }}</view>
		</view>
		<view class="w-item-bg" :style="[itemBarStyle]"></view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-subsection active-color="#ff9900"></w-subsection>
	 */
	export default {
		name: "w-subsection",
		props: {
		...
		computed: {
			...
			noBorderRight() {
				return index => {
					...
					let classs = '';
					// 不显示右边的边框
					if (index < this.list.length - 1) classs += ' w-none-border-right';
					// 显示整个组件的左右边圆角
					if (index == 0) classs += ' w-item-first';
					if (index == this.list.length - 1) classs += ' w-item-last';
					return classs;
				};
			},
			// 文字的样式
		...
		methods: {
			...
			getTabsInfo() {
				...
				for (let i = 0; i < this.list.length; i++) {
					view.select('.w-item-' + i).boundingClientRect();
				}
				view.exec(res => {
					if (!res.length) {
						setTimeout(() => {
