<template>
	<view class="w-tabs" :style="{
		...
		<scroll-view scroll-x class="w-scroll-view" :scroll-left="scrollLeft" scroll-with-animation :style="{ zIndex: zIndex + 1 }">
			<view class="w-tabs-scroll-box" :class="{'w-tabs-scorll-flex': !isScroll}">
				<view class="w-tabs-item" :style="[tabItemStyle(index)]"
				 v-for="(item, index) in getTabs" :key="index" :class="[preId + index]" @tap="emit(index)">
					{{ item[name] || item['name']}}
				</view>
				<view v-if="showBar" class="w-scroll-bar" :style="[tabBarStyle]"></view>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-tabs-swiper ref="tabs" :list="list" :is-scroll="false"></w-tabs-swiper>
	 */
	export default {
		name: "w-tabs-swiper",
		props: {
		...
		methods: {
			...
			getQuery(cb) {
				try {
					let view = uni.createSelectorQuery().in(this).select('.w-tabs');
					view.fields({
							size: true
						},
						data => {
...
<style scoped lang="scss">
	...
	.w-tabs-scorll-flex .w-tabs-item {
		flex: 1;
	}
