<template>
	<view class="w-collapse-item" :style="[itemStyle]">
		<view :hover-stay-time="200" class="w-collapse-head" @tap.stop="headClick" :hover-class="hoverClass" :style="[headStyle]">
			<block v-if="!$slots['title-all']">
				<view v-if="!$slots['title']" class="w-collapse-title w-line-1" :style="[{ textAlign: align ? align : 'left' },
					isShow && activeStyle && !arrow ? activeStyle : '']">
					{{ title }}
				</view>
				<slot v-else name="title" />
				<view class="w-icon-wrap">
					<w-icon v-if="arrow" :color="arrowColor" :class="{ 'w-arrow-down-icon-active': isShow }"
					 class="w-arrow-down-icon" name="arrow-down"></w-icon>
				</view>
			</block>
			<slot v-else name="title-all" />
		</view>
		<view class="w-collapse-body" :style="[{
				height: isShow ? height + 'px' : '0'
			}]">
			<view class="w-collapse-content" :id="elId" :style="[bodyStyle]">
				<slot></slot>
			</view>
		</view>
	</view>
...
<script>
	 ...
	 * @description 通过折叠面板收纳内容区域（搭配w-collapse使用）
	 ...
	 * @example <w-collapse-item :title="item.head" v-for="(item, index) in itemList" :key="index">{{item.body}}</w-collapse-item>
	 */
	export default {
		...
		created() {
			// 获取w-collapse的信息，放在w-collapse是为了方便，不用每个w-collapse-item写一遍
		...
		methods: {
			...
			// 查询内容高度
			queryRect() {
				// $wGetRect为uView自带的节点查询简化方法，详见文档介绍：https://www.uviewui.com/js/getRect.html
				// 组件内部一般用this.$wGetRect，对外的为this.$w.getRect，二者功能一致，名称不同
				this.$wGetRect('#' + this.elId).then(res => {
