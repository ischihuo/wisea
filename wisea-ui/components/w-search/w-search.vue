<template>
	<view class="w-search" @tap="clickHandler" :style="{
		margin: margin,
	}">
		<view
			class="w-content"
		...
		>
			<view class="w-icon-wrap">
				<w-icon class="w-clear-icon" :size="30" :name="searchIcon" :color="searchIconColor ? searchIconColor : color"></w-icon>
			</view>
			<input
				confirm-type="search"
				@blur="blur"
				...
				:focus="focus"
				placeholder-class="w-placeholder-class"
			...
			/>
			<view class="w-close-wrap" v-if="keyword && clearabled && focused" @touchstart="clear">
				<w-icon class="w-clear-icon" name="close-circle-fill" size="34" color="#c0c4cc"></w-icon>
			</view>
		</view>
		<view :style="[actionStyle]" class="w-action" 
			:class="[showActionBtn || show ? 'w-action-active' : '']" 
 ...
 * @example <w-search placeholder="日照香炉生紫烟" v-model="keyword"></w-search>
 */
export default {
	name: "w-search",
	props: {
