<template>
	<view class="w-no-network" v-if="!isConnected" :style="{'z-index': uZIndex}" @touchmove.stop.prevent="">
		<view class="w-inner">
			<image class="w-error-icon" :src="image" mode="widthFix"></image>
			<view class="w-tips">
				{{tips}}
			</view>
			<!-- 只有APP平台，才能跳转设置页，因为需要调用plus环境 -->
			<!-- #ifdef APP-PLUS -->
			<view class="w-to-setting">
				请检查网络，或前往<text class="w-setting-btn" @tap="openSettings">设置</text>
			</view>
			<!-- #endif -->
			<view class="w-retry" :hover-stay-time="150" @tap="retry" hover-class="w-retry-hover">
				重试
			</view>
		</view>
	</view>
...
<script>
	 ...
	 * @example <w-no-network></w-no-network>
	 */
	export default {
		name: "w-no-network",
		props: {
		...
		computed: {
			uZIndex() {
				return this.zIndex ? this.zIndex : this.$w.zIndex.noNetwork;
			}
		},
		mounted() {
