<template>
	<view class="w-numberbox">
		<view class="w-icon-minus" @touchstart.stop="btnTouchStart('minus')" @touchend.stop="clearTimer" :class="{ 'w-icon-disabled': disabled || inputVal <= min }"
		    :style="{
				background: bgColor,
				height: inputHeight + 'rpx',
				color: color
			}">
			<w-icon name="minus" :size="size"></w-icon>
		</view>
		<input :disabled="disabledInput || disabled" :cursor-spacing="getCursorSpacing" :class="{ 'w-input-disabled': disabled }"
		    v-model="inputVal" class="w-number-input" @blur="onBlur"
		    type="number" :style="{
				color: color,
				fontSize: size + 'rpx',
				background: bgColor,
		...
		<view class="w-icon-plus" @touchstart.stop="btnTouchStart('plus')" @touchend.stop="clearTimer" :class="{ 'w-icon-disabled': disabled || inputVal >= max }"
		    :style="{
				background: bgColor,
				height: inputHeight + 'rpx',
				color: color
			}">
			<w-icon name="plus" :size="size"></w-icon>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-number-box :min="1" :max="100"></w-number-box>
	 */
	export default {
		name: "w-number-box",
		props: {
