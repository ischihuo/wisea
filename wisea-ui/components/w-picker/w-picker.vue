<template>
	<w-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto" :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="uZIndex">
		<view class="w-datetime-picker">
			<view class="w-picker-header" @touchmove.stop.prevent="">
				<view class="w-btn-picker w-btn-picker--tips" 
					:style="{ color: cancelColor }" 
					hover-class="w-opacity" 
					:hover-stay-time="150" 
					@tap="getResult('cancel')"
				>{{cancelText}}</view>
				<view class="w-picker__title">{{ title }}</view>
				<view
					class="w-btn-picker w-btn-picker--primary"
			...
			<view class="w-picker-body">
				<picker-view v-if="mode == 'region'" :value="valueArr" @change="change" class="w-picker-view" @pickstart="pickstart" @pickend="pickend">
					<picker-view-column v-if="params.province">
						<view class="w-column-item" v-for="(item, index) in provinces" :key="index">
							<view class="w-line-1">{{ item.label }}</view>
						</view>
					</picker-view-column>
					<picker-view-column v-if="params.city">
						<view class="w-column-item" v-for="(item, index) in citys" :key="index">
							<view class="w-line-1">{{ item.label }}</view>
						</view>
					</picker-view-column>
					<picker-view-column v-if="params.area">
						<view class="w-column-item" v-for="(item, index) in areas" :key="index">
							<view class="w-line-1">{{ item.label }}</view>
						</view>
					</picker-view-column>
				</picker-view>
				<picker-view v-else-if="mode == 'time'" :value="valueArr" @change="change" class="w-picker-view" @pickstart="pickstart" @pickend="pickend">
					<picker-view-column v-if="!reset && params.year">
						<view class="w-column-item" v-for="(item, index) in years" :key="index">
							{{ item }}
							<text class="w-text" v-if="showTimeTag">年</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.month">
						<view class="w-column-item" v-for="(item, index) in months" :key="index">
							{{ formatNumber(item) }}
							<text class="w-text" v-if="showTimeTag">月</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.day">
						<view class="w-column-item" v-for="(item, index) in days" :key="index">
							{{ formatNumber(item) }}
							<text class="w-text" v-if="showTimeTag">日</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.hour">
						<view class="w-column-item" v-for="(item, index) in hours" :key="index">
							{{ formatNumber(item) }}
							<text class="w-text" v-if="showTimeTag">时</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.minute">
						<view class="w-column-item" v-for="(item, index) in minutes" :key="index">
							{{ formatNumber(item) }}
							<text class="w-text" v-if="showTimeTag">分</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && params.second">
						<view class="w-column-item" v-for="(item, index) in seconds" :key="index">
							{{ formatNumber(item) }}
							<text class="w-text" v-if="showTimeTag">秒</text>
						</view>
					</picker-view-column>
				</picker-view>
				<picker-view v-else-if="mode == 'selector'" :value="valueArr" @change="change" class="w-picker-view" @pickstart="pickstart" @pickend="pickend">
					<picker-view-column>
						<view class="w-column-item" v-for="(item, index) in range" :key="index">
							<view class="w-line-1">{{ getItemValue(item, 'selector') }}</view>
						</view>
					</picker-view-column>
				</picker-view>
				<picker-view v-else-if="mode == 'multiSelector'" :value="valueArr" @change="change" class="w-picker-view" @pickstart="pickstart" @pickend="pickend">
					<picker-view-column v-for="(item, index) in range" :key="index">
						<view class="w-column-item" v-for="(item1, index1) in item" :key="index1">
							<view class="w-line-1">{{ getItemValue(item1, 'multiSelector') }}</view>
						</view>
					</picker-view-column>
				</picker-view>
			</view>
 ...
 * @example <w-picker v-model="show" mode="time"></w-picker>
 */
export default {
	name: 'w-picker',
	props: {
