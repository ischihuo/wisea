<template>
	<view class="w-rate" :id="elId" @touchmove.stop.prevent="touchMove">
		<view class="w-star-wrap" v-for="(item, index) in count" :key="index" :class="[elClass]">
			<w-icon
 ...
 * @example <w-rate :count="count" :current="2"></w-rate>
 */
export default {
	name: 'w-rate',
	props: {
	...
	methods: {
		...
		getElRectById() {
			...
			this.$w.getRect('#' + this.elId).then(res => {
		...
		getElRectByClass() {
			// uView封装的获取节点的方法，详见文档
			this.$w.getRect('.' + this.elClass).then(res => {
