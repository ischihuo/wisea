<template>
	<view class="w-char-box">
		<view class="w-char-flex">
			<input :disabled="disabledKeyboard" :value="valueModel" type="number" :focus="focus" :maxlength="maxlength" class="w-input" @input="getVal"/>
			<view v-for="(item, index) in maxlength" :key="index">
				<view :class="[breathe && charArrLength == index ? 'w-breathe' : '', 'w-char-item',
				charArrLength === index && mode == 'box' ? 'w-box-active' : '',
				mode === 'box' ? 'w-box' : '']" :style="{
					...
					<view class="w-placeholder-line" :style="{
							display: charArrLength === index ? 'block' : 'none',
							height: width * 0.5 +'rpx'
						}"
						v-if="mode !== 'middleLine'"
					></view>
					<view v-if="mode === 'middleLine' && charArrLength <= index" :class="[breathe && charArrLength == index ? 'w-breathe' : '', charArrLength === index ? 'w-middle-line-active' : '']"
					 class="w-middle-line" :style="{height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor}"></view>
					<view v-if="mode === 'bottomLine'" :class="[breathe && charArrLength == index ? 'w-breathe' : '', charArrLength === index ? 'w-buttom-line-active' : '']"
					 class="w-bottom-line" :style="{height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor}"></view>
					<block v-if="!dotFill"> {{ charArr[index] ? charArr[index] : ''}}</block>
					<block v-else>
						<text class="w-dot">{{ charArr[index] ? '●' : ''}}</text>
					</block>
				</view>
			</view>
		</view>
...
<script>
	 ...
	 * @example <w-message-input mode="bottomLine"></w-message-input>
	 */
	export default {
		name: "w-message-input",
		props: {
			// 最大输入长度
			maxlength: {
				type: [Number, String],
		...
		computed: {
			...
			animationClass() {
				return (index) => {
					if (this.breathe && this.charArr.length == index) return 'w-breathe';
					else return '';
				}
			},
			// 用于显示字符
...
<style scoped lang="scss">
	...
	.w-box-active {
		...
		border: 2rpx solid $w-type-primary;
	}

	.w-middle-line-active {
		background: $w-type-primary;
	}
