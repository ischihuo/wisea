<template>
	<view class="w-section">
		<view class="w-section__title" :style="{
			fontWeight: bold ? 'bold' : 'normal',
			color: color,
			fontSize: fontSize + 'rpx',
			paddingLeft: showLine ? (fontSize * 0.7) + 'rpx' : 0
		}" :class="{
			'w-section--line': showLine
		}">
			<view class="w-section__title__icon-wrap w-flex" :style="[lineStyle]" v-if="showLine">
				<w-icon top="0" name="column-line" :size="fontSize * 1.25" bold :color="lineColor ? lineColor : color"></w-icon>
			</view>
			<text class="w-flex w-section__title__text">{{title}}</text>
		</view>
		<view class="w-section__right-info" v-if="right" :style="{
			color: subColor
		}" @tap="rightClick">
			{{subTitle}}
			<view class="w-section__right-info__icon-arrow w-flex">
				<w-icon name="arrow-right" size="24" :color="subColor"></w-icon>
			</view>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-section title="今日热门" :right="false"></w-section>
	 */
	export default {
		name: "w-section",
		props: {
		...
		computed: {
			...
			lineStyle() {
				...
				return {
					...
					top: -(Number(this.fontSize) * (this.$w.os == 'ios' ? 0.14 : 0.15)) + 'rpx',
				}
			}
		},
		methods: {
