<template>
	...
	<view>
		<view class="w-index-anchor-wrapper" :id="$w.guid()" :style="[wrapperStyle]">
			<view class="w-index-anchor " :class="[active ? 'w-index-anchor--active' : '']" :style="[customAnchorStyle]">
				<slot v-if="useSlot" />
				<block v-else>
					<text>{{ index }}</text>
				</block>
...
<script>
	 ...
	 * @example <w-index-anchor :index="item" />
	 */
	export default {
		name: "w-index-anchor",
		props: {
