<template>
	<view class="w-switch" :class="[value == true ? 'w-switch--on' : '', disabled ? 'w-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
		<view class="w-switch__node node-class">
			<w-loading :show="loading" class="w-switch__loading" :size="size * 0.6" :color="loadingColor" />
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-switch v-model="checked" active-color="red" inactive-color="#eee"></w-switch>
	 */
	export default {
		name: "w-switch",
		props: {
