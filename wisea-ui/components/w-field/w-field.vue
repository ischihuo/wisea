<template>
	<view class="w-field" :class="{'w-border-top': borderTop, 'w-border-bottom': borderBottom }">
		<view class="w-field-inner" :class="[type == 'textarea' ? 'w-textarea-inner' : '', 'w-label-postion-' + labelPosition]">
			<view class="w-label" :class="[required ? 'w-required' : '']" :style="{
				justifyContent: justifyContent, 
				flex: labelPosition == 'left' ? `0 0 ${labelWidth}rpx` : '1'
			}">
				<view class="w-icon-wrap" v-if="icon">
					<w-icon size="32" :custom-style="iconStyle" :name="icon" :color="iconColor" class="w-icon"></w-icon>
				</view>
				<slot name="icon"></slot>
				<text class="w-label-text" :class="[this.$slots.icon || icon ? 'w-label-left-gap' : '']">{{ label }}</text>
			</view>
			<view class="fild-body">
				<view class="w-flex-1 w-flex" :style="[inputWrapStyle]">
					<textarea v-if="type == 'textarea'" class="w-flex-1 w-textarea-class" :style="[fieldStyle]" :value="value"
					...
					<input
						...
						:style="[fieldStyle]"
						:type="type"
						class="w-flex-1 w-field__input-wrap"
						:value="value"
				...
				<w-icon :size="clearSize" v-if="clearable && value != '' && focused" name="close-circle-fill" color="#c0c4cc" class="w-clear-icon" @touchstart="onClear"/>
				<view class="w-button-wrap"><slot name="right" /></view>
				<w-icon v-if="rightIcon" @click="rightIconClick" :name="rightIcon" color="#c0c4cc" :style="[rightIconStyle]" size="26" class="w-arror-right" />
			</view>
		</view>
		<view v-if="errorMessage !== false && errorMessage != ''" class="w-error-message" :style="{
 ...
 * @example <w-field v-model="mobile" label="手机号" required :error-message="errorMessage"></w-field>
 */
export default {
	name:"w-field",
	props: {
