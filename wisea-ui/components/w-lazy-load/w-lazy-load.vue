<template>
	<view class="w-wrap" :style="{
			  opacity: Number(opacity),
			  borderRadius: borderRadius + 'rpx',
			  // 因为time值需要改变,所以不直接用duration值(不能改变父组件prop传过来的值)
			  transition: `opacity ${time / 1000}s ease-in-out`
		   }"
	 :class="'w-lazy-item-' + elIndex">
		<view :class="'w-lazy-item-' + elIndex">
			<image :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" v-if="!isError" class="w-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" class="w-lazy-item error" v-else :src="errorImg"
...
<script>
	 ...
	 * @example <w-lazy-load :image="image" :loading-img="loadingImg" :error-img="errorImg"></w-lazy-load>
	 */
	export default {
		name: 'w-lazy-load',
		props: {
		...
		mounted() {
			...
			setTimeout(() => {
				...
				}).observe('.w-lazy-item-' + this.elIndex, (res) => {
