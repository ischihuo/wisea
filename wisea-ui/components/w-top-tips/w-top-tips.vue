<template>
	<view class="w-tips" :class="['w-' + type, isShow ? 'w-tip-show' : '']" :style="{
		top: navbarHeight + 'px',
		zIndex: uZIndex
	}">{{ title }}</view>
</template>
...
<script>
	 ...
	 * @example <w-top-tips ref="uTips"></w-top-tips>
	 */
	export default {
		name: "w-top-tips",
		props: {
		...
		computed: {
			uZIndex() {
				return this.zIndex ? this.zIndex : this.$w.zIndex.topTips;
			}
		},
		methods: {
			show(config = {}) {
