<template>
	<view class="">
		<movable-area class="w-swipe-action" :style="{ backgroundColor: bgColor }">
			<movable-view
				class="w-swipe-view"
				@change="change"
				@touchend="touchend"
			...
			>
				<view
					class="w-swipe-content"
					@tap.stop="contentClick"
				>
					<slot></slot>
				</view>
				<view class="w-swipe-del" v-if="showBtn" @tap.stop="btnClick(index)" :style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<view class="w-btn-text">{{ item.text }}</view>
				</view>
			</movable-view>
		</movable-area>
	</view>
 ...
 * @example <w-swipe-action btn-text="收藏">...</w-swipe-action>
 */
export default {
	name: 'w-swipe-action',
	props: {
	...
	data() {
		return {
			...
			elId: this.$w.guid(), // id，用于通知另外组件关闭时的识别
	...
	methods: {
		...
		touchstart() {},
		getActionRect() {
			this.$wGetRect('.w-swipe-action').then(res => {
