<template>
	<view class="w-radio" :style="[radioStyle]">
		<view class="w-radio__icon-wrap" @tap="toggle" :class="[iconClass]" :style="[iconStyle]">
			<w-icon
			    name="checkbox-mark"
			    :size="elIconSize" 
				:color="iconColor"/>
		</view>
		<view class="w-radio__label" @tap="onClickLabel" :style="{
...
<script>
	/**
	 * radio 单选框
	 * @description 单选框用于有一个选择，用户只能选择其中一个的场景。搭配w-radio-group使用
	 ...
	 * @example <w-radio :label-disabled="false">门掩黄昏，无计留春住</w-radio>
	 */
	export default {
		name: "w-radio",
		props: {
		...
		created() {
			...
			this.parent = this.$w.$parent.call(this, 'w-radio-group');
		},
		computed: {
			// 是否禁用，如果父组件w-raios-group禁用的话，将会忽略子组件的配置
			elDisabled() {
			...
			iconClass() {
				let classes = [];
				classes.push('w-radio__icon-wrap--' + this.elShape);
				if (this.name == this.parent.value) classes.push('w-radio__icon-wrap--checked');
				if (this.elDisabled) classes.push('w-radio__icon-wrap--disabled');
				if (this.name == this.parent.value && this.elDisabled) classes.push(
					'w-radio__icon-wrap--disabled--checked');
		...
		methods: {
			...
			emitEvent() {
				// w-radio的name不等于父组件的v-model的值时(意味着未选中)，才发出事件，避免多次点击触发事件
