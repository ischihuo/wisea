<template>
	<view class="">
		<view class="w-content" :class="[elId]" :style="{ 
			height: isLongContent && !showMore ? showHeight + 'rpx' : 'auto',
			textIndent: this.textIndent
		}">
			<slot></slot>
		</view>
		<view @tap="toggleReadMore" v-if="isLongContent" class="w-content__showmore-wrap"
		    :class="{ 'w-content__show-more': showMore }"
		    :style="[innerShadowStyle]">
			<text class="w-content__showmore-wrap__readmore-btn" :style="{
				fontSize: fontSize + 'rpx',
				color: color
			}">
				{{ showMore ? openText : closeText }}
			</text>
			<view class="w-content__showmore-wrap__readmore-btn__icon w-flex">
				<w-icon :color="color" :size="fontSize" :name="showMore ? 'arrow-up' : 'arrow-down'"></w-icon>
			</view>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-read-more><rich-text :nodes="content"></rich-text></w-read-more>
	 */
	export default {
		name: "w-read-more",
		props: {
