 ...
 * @example <w-circle-progress active-color="#2979ff" :percent="80"></w-circle-progress>
 */
export default {
	name: 'w-circle-progress',
	props: {
		// 圆环进度百分比值
		percent: {
			type: Number,
	...
	data() {
		return {
			// #ifdef MP-WEIXIN
			elBgId: 'uCircleProgressBgId', // 微信小程序中不能使用this.$w.guid()形式动态生成id值，否则会报错
			elId: 'uCircleProgressElId',
			// #endif
			// #ifndef MP-WEIXIN
			elBgId: this.$w.guid(), // 非微信端的时候，需用动态的id，否则一个页面多个圆形进度条组件数据会混乱
			elId: this.$w.guid(),
			// #endif
	...
	computed: {
		// 有type主题时，优先起作用
		circleColor() {
			if (['success', 'error', 'info', 'primary', 'warning'].indexOf(this.type) >= 0) return this.$w.color[this.type];
