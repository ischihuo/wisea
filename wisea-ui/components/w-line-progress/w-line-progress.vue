<template>
	<view class="w-progress" :style="{
		...
		<view :class="[
			type ? `w-type-${type}-bg` : '',
			striped ? 'w-striped' : '',
			striped && stripedActive ? 'w-striped-active' : ''
		]" class="w-active" :style="[progressStyle]">
...
<script>
	 ...
	 * @example <w-line-progress :percent="70" :show-percent="true"></w-line-progress>
	 */
	export default {
		name: "w-line-progress",
		props: {
			// 两端是否显示半圆形
			round: {
				type: Boolean,
