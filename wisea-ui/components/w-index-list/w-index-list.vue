<template>
	<!-- 支付宝小程序使用$w.getRect()获取组件的根元素尺寸，所以在外面套一个"壳" -->
	<view>
		<view class="w-index-bar">
			<slot />
			<view v-if="showSidebar" class="w-index-bar__sidebar" @touchstart.stop.prevent="onTouchMove" @touchmove.stop.prevent="onTouchMove"
			 @touchend.stop.prevent="onTouchStop" @touchcancel.stop.prevent="onTouchStop">
				<view v-for="(item, index) in indexList" :key="index" class="w-index-bar__index" :style="{zIndex: zIndex + 1, color: activeAnchorIndex === index ? activeColor : ''}"
				 :data-index="index">
					{{ item }}
				</view>
			</view>
			<view class="w-indexed-list-alert" v-if="touchmove && indexList[touchmoveIndex]" :style="{
				zIndex: alertZIndex
			}">
...
<script>
	...
	/**
	 * indexList 索引列表
	 * @description 通过折叠面板收纳内容区域,搭配<w-index-anchor>使用
	 ...
	 * @example <w-index-list :scrollTop="scrollTop"></w-index-list>
	 */
	export default {
		name: "w-index-list",
		props: {
		...
		methods: {
			...
			setAnchorsRect() {
				return Promise.all(this.children.map((anchor, index) => anchor
					.$wGetRect('.w-index-anchor-wrapper')
					.then((rect) => {
			...
			setListRect() {
				return this.$wGetRect('.w-index-bar').then((rect) => {
					Object.assign(this, {
						height: rect.height,
			...
			setSiderbarRect() {
				return this.$wGetRect('.w-index-bar__sidebar').then(rect => {
					this.sidebar = {
						height: rect.height,
						top: rect.top
					};
						...
						if (index === active) {
							...
							if (isActiveAnchorSticky) {
								...
								anchorStyle = {
									...
									zIndex: `${zIndex ? zIndex : this.$w.zIndex.indexListSticky}`,
									color: `${activeColor}`
								};
							}
							item.active = active;
