<template>
	<view class="w-avatar" :style="[wrapStyle]" @tap="click">
		<image
			@error="loadError"
			:style="[imgStyle]"
			class="w-avatar__img"
			...
			:mode="mode"
		></image>
		<text class="w-line-1" v-else-if="uText" :style="{
			fontSize: '38rpx'
		}">{{uText}}</text>
		<slot v-else></slot>
		<view class="w-avatar__sex" v-if="showSex" :class="['w-avatar__sex--' + sexIcon]" :style="[uSexStyle]">
			<w-icon :name="sexIcon" size="20"></w-icon>
		</view>
		<view class="w-avatar__level" v-if="showLevel" :style="[uLevelStyle]">
			<w-icon :name="levelIcon" size="20"></w-icon>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <w-avatar :src="src"></w-avatar>
	 */
	export default {
		name: 'w-avatar',
		props: {
