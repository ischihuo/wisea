<template>
	<view class="w-image" @tap="onClick" :style="[wrapStyle, backgroundStyle]">
		<image
			v-if="!isError"
			:src="src"
			:mode="mode"
			...
			@load="onLoadHandler"
			:lazy-load="lazyLoad"
			class="w-image__image"
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $w.addUnit(borderRadius)
			}"
		></image>
		<view
			v-if="showLoading && loading"
			class="w-image__loading"
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $w.addUnit(borderRadius),
		...
		>
			...
			<w-icon v-else :name="loadingIcon" :width="width" :height="height"></w-icon>
		</view>
		<view
			v-if="showError && isError && !loading"
			class="w-image__error"
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $w.addUnit(borderRadius)
			}"
		>
			<slot v-if="$slots.error" name="error" />
			<w-icon v-else :name="errorIcon" :width="width" :height="height"></w-icon>
		</view>
	</view>
</template>
 ...
 * @property {Boolean} show-menw-by-longpress 是否开启长按图片显示识别小程序码菜单，仅微信小程序有效（默认 false）
 ...
 * @example <w-image width="100%" height="300rpx" :src="src"></w-image>
 */
export default {
	name: 'w-image',
	props: {
	...
	computed: {
		wrapStyle() {
			...
			style.borderRadius = this.shape == 'circle' ? '50%' : this.$w.addUnit(this.borderRadius);
