<template>
	<view class="w-upload" v-if="!disabled">
		<view
			v-if="showUploadList"
			class="w-list-item w-preview-wrap"
		...
		>
			...
			>
				<w-icon class="w-icon" :name="delIcon" size="20" :color="delColor"></w-icon>
			</view>
			<w-line-progress
			...
			></w-line-progress>
			<view @tap.stop="retry(index)" v-if="item.error" class="w-error-btn">点击重试</view>
			<image @tap.stop="doPreviewImage(item.url || item.path, index)" class="w-preview-image" v-if="!item.isImage" :src="item.url || item.path" :mode="imageMode"></image>
		</view>
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			<view
				v-if="!customBtn"
				class="w-list-item w-add-wrap"
				hover-class="w-add-wrap__hover"
				hover-stay-time="150"
				:style="{
					width: width + 'rpx',
					height: width + 'rpx'
			...
			>
				<w-icon name="plus" class="w-add-btn" size="40"></w-icon>
				<view class="w-add-tips">{{ uploadText }}</view>
			</view>
		</view>
	</view>
</template>
 ...
 * @example <w-upload :action="action" :file-list="fileList" ></w-upload>
 */
export default {
	name: 'w-upload',
	props: {
		...
		async uploadFile(index = 0) {
			...
			const task = uni.uploadFile({
				...
				success: res => {
					...
					let data = this.toJson && this.$w.test.jsonString(res.data) ? JSON.parse(res.data) : res.data;
