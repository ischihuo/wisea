<template>
	<view class="">
		<view class="w-sticky-wrap" :class="[elClass]" :style="{
			height: fixed ? height + 'px' : 'auto',
			backgroundColor: bgColor
		}">
			<view class="w-sticky" :style="{
...
<script>
	 ...
	 * @example <w-sticky offset-top="200"><view>塞下秋来风景异，衡阳雁去无留意</view></w-sticky>
	 */
	export default {
		name: "w-sticky",
		props: {
		...
		computed: {
			uZIndex() {
				return this.zIndex ? this.zIndex : this.$w.zIndex.sticky;
			}
		},
		mounted() {
			this.initObserver();
		...
		methods: {
			initObserver() {
				...
				this.disconnectObserver('contentObserver');
				this.$wGetRect('.' + this.elClass).then((res) => {
