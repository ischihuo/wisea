<template>
  <view class="dropdown-list-wapper w-flex w-flex-1">
    <view
      v-for="(drop, index) in dropdownListFromFather"
      :key="drop.name"
      :show="drop.show"
      class="w-selected-class w-dropdown-list"
      :style="{ zIndex: zIndex + 1 }"
    >
      <slot name="selectionbox">
        <view
          :style="{ height: top + 'rpx' }"
          class="drop-item w-flex w-justify-center"
        ...
        >
          ...
          <view
            class="w-animation"
            :class="[drop.show ? 'w-animation-show' : '']"
          >
            <w-icon
              v-if="drop.show"
              name="arrow-up-fill"
              ...
              :color="activeColor"
            ></w-icon>
            <w-icon v-else name="arrow-down-fill" :size="18"></w-icon>
          </view>
        </view>
      </slot>
      <view
        class="w-dropdown-view"
        :class="[drop.show ? 'w-dropdownlist-show' : '']"
        :style="{
          background: bgcolor,
      ...
      >
        <slot name="dropdownbox">
          <view class="w-selected-list">
            <view
              class="select-item w-flex w-align-center w-border-bottom w-align-between"
