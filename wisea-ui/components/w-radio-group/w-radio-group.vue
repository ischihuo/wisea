<template>
	<view class="w-radio-group w-clearfix">
		<slot></slot>
	</view>
</template>
...
<script>
	...
	/**
	 * radioRroup 单选框父组件
	 * @description 单选框用于有一个选择，用户只能选择其中一个的场景。搭配w-radio使用
	 ...
	 * @example <w-radio-group v-model="value"></w-radio-group>
	 */
	export default {
		name: "w-radio-group",
		mixins: [Emitter],
		props: {
			...
			// 每个checkbox占w-checkbox-group的宽度
			width: {
				type: String,
				default: 'auto'
			},
		...
		methods: {
			...
			setValue(val) {
				...
				this.$nextTick(function() {
					...
					this.dispatch('w-form-item', 'on-form-change', val);
				});
			}
		}
	}
