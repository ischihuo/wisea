	...
	export default {
		...
		methods: {
			...
			procCalendar(item) {
				...
				item.tapType = true
				if(this.endDate){
					if(new Date(item.dateObj).getTime()<new Date(this.endDate).getTime()+86400000){
					}else{
						item.statusStyle.background = '#eee';
						item.statusStyle.opacity = 1;
						item.tapType = false
						item.statusStyle.color = item.isOtherMonth ? '#999' : '#000'
					}
					// if(this.$u.timeFormat(item.dateObj,'yyyy-mm-dd')==this.endDate){
					// 	item.statusStyle.background = 'transparent';
					// 	item.statusStyle.opacity = 1;
					// 	item.tapType = true
					// }
				}else if(this.beginDate){
					if(new Date(item.dateObj).getTime()>new Date(this.beginDate).getTime()){
					}else{
						item.statusStyle.background = '#eee';
						item.statusStyle.opacity = 1;
						item.tapType = false
