<template>
	<view>
		<view class="wrap">
			<view class="w-tabs-box">
				<w-tabs-swiper activeColor="#f29100" ref="tabs" :list="list" :current="current" @change="change" :is-scroll="false" swiperWidth="750"></w-tabs-swiper>
			</view>
					...
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							<view class="order" v-for="(res, index) in orderList[0]" :key="res.id">
								<view class="top">
									<view class="left">
										<w-icon name="home" :size="30" color="rgb(94,94,94)"></w-icon>
										<view class="store">{{ res.store }}</view>
										<w-icon name="arrow-right" color="rgb(203,203,203)" :size="26"></w-icon>
									</view>
								...
								<view class="item" v-for="(item, index) in res.goodsList" :key="index">
									...
									<view class="content">
										<view class="title w-line-2">{{ item.title }}</view>
								...
								<view class="bottom">
									<view class="more"><w-icon name="more-dot-fill" color="rgb(203,203,203)"></w-icon></view>
									<view class="logistics btn">查看物流</view>
									<view class="exchange btn">卖了换钱</view>
									<view class="evaluate btn">评价</view>
								</view>
							</view>
							<w-loadmore :status="loadStatus[0]" bgColor="#f2f2f2"></w-loadmore>
						</view>
					</scroll-view>
				</swiper-item>
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							...
							<w-loadmore :status="loadStatus[1]" bgColor="#f2f2f2"></w-loadmore>
						</view>
					</scroll-view>
				</swiper-item>
				<swiper-item class="swiper-item">
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							...
							<w-loadmore :status="loadStatus[3]" bgColor="#f2f2f2"></w-loadmore>
						</view>
					</scroll-view>
				</swiper-item>
			</swiper>
...
export default {
	...
	methods: {
		...
		getOrderList(idx) {
			for(let i = 0; i < 5; i++) {
				let index = this.$w.random(0, this.dataList.length - 1);
...
.order {
	...
	.bottom {
		...
		.btn {
			...
			width: 160rpx;
			border-radius: 26rpx;
			border: 2rpx solid $w-border-color;
			font-size: 26rpx;
			text-align: center;
			color: $w-type-info-dark;
		}
		.evaluate {
			color: $w-type-warning-dark;
			border-color: $w-type-warning-dark;
		}
	}
}
.centre {
