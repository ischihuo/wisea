	...
	<view class="w-demo">
		<view class="w-demo-wrap" style="background-color: #ffffff;">
			...
			<view class="w-demo-area">
				...
				<w-collapse :item-style="itemStyle" event-type="close" :arrow="arrow" :accordion="accordion" @change="change">
					<w-collapse-item :index="index" @change="itemChange" :title="item.head" v-for="(item, index) in itemList" :key="index">
						{{item.body}}
					</w-collapse-item>
				</w-collapse>
			</view>
		...
		<view class="w-config-wrap">
			...
			<view class="w-config-item">
				<view class="w-item-title">手风琴模式</view>
				<w-subsection vibrateShort :list="['是', '否']" @change="accordionChange"></w-subsection>
			</view>
			<view class="w-config-item">
				<view class="w-item-title">右侧箭头</view>
				<w-subsection vibrateShort :list="['显示', '隐藏']" @change="arrowChange"></w-subsection>
			</view>
			<view class="w-config-item">
